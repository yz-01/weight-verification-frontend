...
export interface IntegrationPayload { company: string; api_name: string; endpoint?: string; integration_date: string; test_status?: TestStatus; live_status?: ActivationStatus; test_result?: string; notes?: string; }
export interface DevicePayload { company: string; type: string; device_type: string; device_id: string; installed_on?: string | null; installation_location?: string; test_status?: TestStatus; activation_status?: ActivationStatus; firmware_version?: string; sim_status?: string; signal_strength?: number | null; operation_mode?: OperationMode; scheduled_date: string; description: string; }
...
export async function completeMaintenance(id: string, payload: { work_performed: string; result_notes?: string; test_status?: TestStatus; activation_status?: ActivationStatus; next_maintenance_date?: string | null }) { const row = await api.post<DeviceMaintenance>(`/api/device-maintenance/${id}/complete_maintenance/`, payload); toastSuccess("support.toast.updated"); return row; }
