...
/**
 * Attach more evidence to a deduction already raised.
 *
 * Only the recycler that raised it may add to it, and the backend enforces
 * that. This exists because the argument about a deduction usually happens
 * after it is raised: the producer disputes it, and the yard has another
 * photo of the contamination that nobody had uploaded. Without this the only
 * way to add that photo was to void the deduction and raise it again.
 */
export async function addDeductionPhoto(
  id: string,
  image: File,
  caption: string,
): Promise<void> {
  ...
  await api.post(`/api/deductions/${id}/add_photo/`, body);
  toastSuccess("deductions.toast.photoAdded");
}
