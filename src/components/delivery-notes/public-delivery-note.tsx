  ...
  return (
    <main className="mx-auto min-h-dvh max-w-xl bg-background px-4 py-5 pb-12">
      ...
      {!isStopped && !isComplete && noteData.status === "ARRIVED" && <>
        ...
        <section className="mt-6 space-y-4 rounded-xl border bg-card p-4"><h2 className="font-semibold">{t("decisionTitle")}</h2><div className="grid grid-cols-2 gap-2"><Button type="button" variant={decision === "RECEIVED" ? "default" : "outline"} onClick={() => setDecision("RECEIVED")}><CheckCircle2 />{t("action.receive")}</Button><Button type="button" variant={decision === "REJECTED" ? "destructive" : "outline"} onClick={() => setDecision("REJECTED")}><XCircle />{t("action.reject")}</Button></div><FieldWrapper label={t("field.quantity")} required error={!quantity && complete.isError ? t("error.quantityRequired") : undefined}><Input inputMode="decimal" type="number" min="0" step="0.001" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></FieldWrapper><FieldWrapper label={t("field.receiver")} required><Input value={receiver} onChange={(event) => setReceiver(event.target.value)} /></FieldWrapper>{decision === "REJECTED" && <FieldWrapper label={t("field.rejectionReason")} required><Textarea value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} /></FieldWrapper>}<FieldWrapper label={t("field.note")} optional={t("optional")}><Textarea value={note} onChange={(event) => setNote(event.target.value)} /></FieldWrapper><FieldSignaturePad label={t("field.signature")} clearLabel={t("action.clearSignature")} value={signature} onChange={setSignature} /><Button size="lg" className="h-14 w-full text-base" disabled={!evidenceComplete || !quantity || !receiver.trim() || !signature || (decision === "REJECTED" && !rejectionReason.trim()) || complete.isPending} onClick={() => complete.mutate()}>{complete.isPending ? <Loader2 className="animate-spin" /> : <PackageCheck />}{evidenceComplete ? t("action.submit") : t("action.photosRequired")}</Button></section>
      </>}
      {isComplete && <section className="mt-8 rounded-xl border border-success/30 bg-success/5 p-6 text-center"><CheckCircle2 className="mx-auto size-12 text-success" /><h2 className="mt-3 text-lg font-semibold">{t(`decision.${noteData.decision}`)}</h2><p className="mt-2 text-sm text-muted-foreground">{t("completeBody")}</p></section>}
    </main>
  );
}
