...
export function ConsultantApplicationDetail({ id }: { id: string }) {
  ...
  const [decision, setDecision] = useState<
    "APPROVE" | "REJECT" | "REVISE_RESUBMIT" | null
  >(null);
...
function DecisionDialog({ application, decision, onClose, onSaved }: { application: ConsultantApplication; decision: "APPROVE" | "REJECT" | "REVISE_RESUBMIT"; onClose: () => void; onSaved: () => void }) {
  const t = useTranslations("consultantWorkflow");
...
function statusTone(status: ConsultantApplication["status"]): "neutral" | "positive" | "warning" | "danger" | "info" {
  ...
  if (status === "SUBMITTED" || status === "REVISE_RESUBMIT") return "warning";
