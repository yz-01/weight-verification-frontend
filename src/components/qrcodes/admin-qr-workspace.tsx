...
/** One definition for the company options, shared by the register filter and the issue dialog. */
const companyOptionsQuery = {
...
function QRCodeRegister({
  ...
  const companies = useQuery(companyOptionsQuery);
    ...
    <div className="space-y-4">
      ...
      <Table>
        ...
        <TableBody>
          ...
          {codes.isError ? <FailedRow columns={9} query={codes} what={t("what.codes")} /> : !codes.isLoading && (codes.data?.results.length ?? 0) === 0 && <EmptyRow columns={9} />}
        </TableBody>
      </Table>
      <Pagination page={page} totalPages={codes.data?.total_pages ?? 0} count={codes.data?.count} onPage={setPage} />
      <IssueDialog open={showIssue} onClose={() => setShowIssue(false)} onIssued={(code) => { setShowIssue(false); setIssued(code); void queryClient.invalidateQueries({ queryKey: ["admin-qr"] }); }} />
...
function IssueDialog({ open, onClose, onIssued }: { open: boolean; onClose: () => void; onIssued: (code: QRCodeIssue) => void }) {
  const t = useTranslations("adminQr");
  const common = useTranslations("common");
  const companyOptions = useQuery({ ...companyOptionsQuery, enabled: open });
...
function QRStatistics() {
  ...
  // Without an answer every figure is a dash: a failed read is not "0 codes".
  const none = common("emptyValue");
  return <div className="space-y-5"><QueryFailedNote query={summary} what={t("what.summary")} /><div className="grid border-l border-t sm:grid-cols-2 lg:grid-cols-4">{[["total", data ? data.total : none], ["active", data ? (data.by_status.ACTIVE ?? 0) : none], ["scansToday", data ? data.scans_today : none], ["failedScansToday", data ? data.failed_scans_today : none]].map(([key, value]) => <div key={key} className="min-h-24 border-b border-r p-4"><p className="text-xs text-muted-foreground">{t(`metric.${key}`)}</p><p className="mt-3 text-xl font-semibold tabular-nums">{value}</p></div>)}</div><div className="grid gap-5 lg:grid-cols-2"><div><h3 className="mb-3 text-sm font-semibold">{t("statistics.byType")}</h3><div className="divide-y border-y">{SUBJECT_TYPES.map((type) => <div key={type} className="flex items-center justify-between py-3"><span>{t(`subjectType.${type}`)}</span><span className="tabular-nums font-medium">{data ? (data.by_subject_type[type] ?? 0) : none}</span></div>)}</div></div><div><h3 className="mb-3 text-sm font-semibold">{t("statistics.byStatus")}</h3><div className="divide-y border-y">{(["ACTIVE", "DISABLED", "VOIDED", "EXPIRED", "SUPERSEDED"] as QRCodeStatus[]).map((status) => <div key={status} className="flex items-center justify-between py-3"><StatusBadge label={t(`status.${status}`)} tone={statusTone(status)} /><span className="tabular-nums font-medium">{data ? (data.by_status[status] ?? 0) : none}</span></div>)}</div></div></div></div>;
}
...
function Pagination({ page, totalPages, count, onPage }: { page: number; totalPages: number; count?: number; onPage: (page: number) => void }) {
  ...
  return <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">{count === undefined ? common("emptyValue") : t("count", { count })}</span><div className="flex gap-2"><Button size="sm" variant="outline" disabledReason={page <= 1 ? common("alreadyFirstPage") : undefined} disabled={page <= 1} onClick={() => onPage(page - 1)}>{table("previous")}</Button><Button size="sm" variant="outline" disabled={page >= totalPages} onClick={() => onPage(page + 1)}>{table("next")}</Button></div></div>;
}
...
/** A table body row that says the read failed, so it never reads as "no records". */
function FailedRow({ columns, query, what }: { columns: number; query: { isError: boolean; refetch?: () => unknown }; what: string }) {
  return <TableRow><TableCell colSpan={columns} className="h-28 text-center"><QueryFailedNote query={query} what={what} className="justify-center" /></TableCell></TableRow>;
}
