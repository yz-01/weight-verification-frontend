...
function LifecycleDialog({ selection, note, setNote, pending, onClose, onSave }: { selection: { code: QRCode; status?: "ACTIVE" | "DISABLED" | "VOIDED"; reissue?: boolean } | null; note: string; setNote: (value: string) => void; pending: boolean; onClose: () => void; onSave: () => void }) {
  ...
  return <Dialog open={selection !== null} onOpenChange={(open) => !open && onClose()}><DialogContent><DialogHeader><DialogTitle>{t(`action.${action}`)}</DialogTitle><DialogDescription>{selection?.code.serial}</DialogDescription></DialogHeader><FieldWrapper label={t("field.reason")} required={action === "void" || action === "reissue"}><Textarea value={note} onChange={(event) => setNote(event.target.value)} /></FieldWrapper><DialogFooter><Button variant="outline" onClick={onClose}>{common("cancel")}</Button><Button variant={action === "void" ? "destructive" : "default"} disabled={pending || ((action === "void" || action === "reissue") && !note.trim())} onClick={onSave}>{common("confirm")}</Button></DialogFooter></DialogContent></Dialog>;
}
