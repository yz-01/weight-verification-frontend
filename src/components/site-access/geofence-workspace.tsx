...
function GeofenceDialog({ row, defaultProject, projects, onClose, onSaved }: { row: SiteGeofence | null; defaultProject: string; projects: Array<{ id: string; latitude?: string | null; longitude?: string | null; geofence_radius_m: number | null }>; onClose: () => void; onSaved: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[92dvh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{t(row ? "geofence.editTitle" : "geofence.createTitle")}</DialogTitle><DialogDescription>{t("geofence.formHelp")}</DialogDescription></DialogHeader>
    ...
    <DialogFooter><Button variant="outline" onClick={onClose}>{t("action.cancel")}</Button><Button disabled={!valid || save.isPending} onClick={() => save.mutate()}><CircleDot />{t("action.save")}</Button></DialogFooter>
