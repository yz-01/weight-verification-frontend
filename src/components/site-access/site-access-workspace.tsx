...
function CredentialsPanel({ pass }: { pass: SiteAccessPass }) {
  ...
  const [type, setType] = useState<AccessCredentialType>("RFID");
  ...
  const revoke = useMutation({
    mutationFn: (credential: SiteAccessCredential) =>
      revokeSiteAccessCredential(pass.id, credential.id),
    onSuccess: async () => {
      setRevoking(null);
      await refresh();
    },
    ...
    <section className="mt-2 space-y-3">
      ...
      {!approved ? (
        ...
        <div className="grid gap-2 rounded-lg border p-3 sm:grid-cols-[10rem_1fr_1fr_auto]">
          <Select
            value={type}
            onValueChange={(next) => setType(next as AccessCredentialType)}
          >
            <SelectTrigger aria-label={t("credential.type")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {(["ANPR", "RFID", "FACE", "VISITOR_ID"] as const).map((kind) => (
          ...
          />
          <Button
            disabled={!value.trim() || register.isPending}
