...
function ShipmentDialog({ shipment, onClose }: { shipment: OutboundShipment | null; onClose: () => void }) {
  ...
  const valid = form.buyer && Number(form.weight_kg) > 0 && form.outbound_date;
  ...
  </div><DialogFooter><Button variant="outline" onClick={onClose}><X />{common("cancel")}</Button><Button disabled={!valid || save.isPending} onClick={() => save.mutate()}>{save.isPending ? <Loader2 className="animate-spin" /> : <CheckCircle2 />}{common("save")}</Button></DialogFooter></DialogContent></Dialog>;
}
