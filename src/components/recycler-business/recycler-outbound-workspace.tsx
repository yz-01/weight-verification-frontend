...
function BuyerDialog({ buyer, onClose }: { buyer: Buyer | null; onClose: () => void }) {
  ...
  const field = (key: keyof BuyerPayload, label: string) => <FieldWrapper label={label}><Input value={form[key] ?? ""} onChange={(event) => set(key, event.target.value)} /></FieldWrapper>;
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>{t(buyer ? "buyers.editTitle" : "buyers.createTitle")}</DialogTitle><DialogDescription>{t("buyers.formHelp")}</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><FieldWrapper label={t("field.companyName")} required><Input value={form.company_name ?? ""} onChange={(event) => set("company_name", event.target.value)} /></FieldWrapper>{field("registration_no", t("field.registrationNo"))}{field("contact_person", t("field.contactPerson"))}{field("contact_phone", t("field.phone"))}{field("contact_email", t("field.email"))}{field("address", t("field.address"))}<FieldWrapper label={t("field.notes")} className="sm:col-span-2"><Textarea value={form.notes ?? ""} onChange={(event) => set("notes", event.target.value)} /></FieldWrapper></div><DialogFooter><Button variant="outline" onClick={onClose}><X />{common("cancel")}</Button><Button requires={[[form.company_name, t("field.companyName")]]} disabled={save.isPending} onClick={() => save.mutate()}>{save.isPending ? <Loader2 className="animate-spin" /> : <CheckCircle2 />}{common("save")}</Button></DialogFooter></DialogContent></Dialog>;
}
