 ...
 */
export const BRANDING_BOOTSTRAP_SCRIPT = `(()=>{try{const p=location.pathname;const f=p.startsWith('/field-staff')||p.startsWith('/trace/field-')||p.startsWith('/field-pwa-bootstrap');const raw=localStorage.getItem(f?'${FIELD_BRANDING_KEY}':'${STANDARD_BRANDING_KEY}');const b=raw?JSON.parse(raw):{name:'MSE Trace',icon_url:'/mse-icon-192.png',revision:null,company_id:null};if(typeof b.name==='string'&&b.name.trim())document.title=b.name.trim();let u;if(typeof b.revision==='string'&&b.revision&&typeof b.icon_url==='string'&&b.icon_url){u=new URL('/brand-icon/192',location.origin);if(typeof b.company_id==='string'&&b.company_id)u.searchParams.set('company',b.company_id);u.searchParams.set('brand',b.revision)}else if(typeof b.icon_url==='string'&&b.icon_url){u=new URL(b.icon_url,location.origin)}else{return}if(u.protocol!=='http:'&&u.protocol!=='https:')return;for(const rel of ['icon','apple-touch-icon']){document.head.querySelectorAll('link[rel="'+rel+'"]').forEach(n=>n.remove());const l=document.createElement('link');l.rel=rel;l.href=u.href;l.dataset.mseBranding=rel;if(rel==='icon')l.sizes='any';else l.sizes='180x180';document.head.appendChild(l)}}catch{}})();`;
